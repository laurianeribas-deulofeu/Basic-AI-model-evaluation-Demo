...
# ---------------------------------------------------------
#setwd(dirname(rstudioapi::getActiveDocumentContext()$path)) #automatically set your working directory as the folder in which this Rscript is located #this line is silenced in Markdown as it is already handled in previous chunk containing the knitr option, if you run from the console of Rscript, run this line
...
```{r training-barplot, fig.height=8}
cs_raw %>%
...
```{r cm-heatmap, fig.height=14, fig.width=15}
...
> **Reading this matrix:** Each row is a true class. Each cell shows the proportion of that class predicted as each column class. The diagonal = recall. Off-diagonal hotspots reveal which classes are confused with which.
