...
# ---------------------------------------------------------
# Set up working directory as the folder in which both Rscript and dataset are located
# ---------------------------------------------------------
setwd(dirname(rstudioapi::getActiveDocumentContext()$path)) #automatically set your working directory as the folder in which this Rscript is located

cm_path<- "./confusion_matrix_full_0.csv" #since you automatically set up your working directory in the previous line,it simplifies calling the confusion matrix and other datasets as long as they are in the same folder as your Rscript
classsize_path<- "./Confusion_matrix_classsize.csv"

# ---------------------------------------------------------

# Load confusion matrix — first column is class names
cm_raw <- read.csv(cm_path) #cm=confusion matrix
class_names <- cm_raw[[1]]
cm_mat <- as.matrix(cm_raw[, -1])
rownames(cm_mat) <- class_names #in case your header does not match the one used in this script + if column name contained certain characters, R will modify these characters and row and col names won't match anymore
colnames(cm_mat) <- class_names

# Load class sizes — expects columns: Class, n_annotations
cs_raw <- read.csv(classsize_path) #cs=class size
colnames(cs_raw) <- c("Class", "n_annotations") #in case your header does not match the one used in this script
...
cs_raw %>%
  ...
  rename(`Class` = Class,
         ...
         `% of total annotations` = Proportion) %>%
...
# Performance Metrics
# Per-class: TP (true positive), FP (false positive), FN (false negative), TN (true negative)
TP <- diag(cm_mat)
FP <- colSums(cm_mat) - TP
FN <- rowSums(cm_mat) - TP
TN <- sum(cm_mat) - TP - FP - FN

precision <- ifelse((TP + FP) == 0, NA, TP / (TP + FP)) # measure the correct detection, give you information for false alarm rate
recall    <- ifelse((TP + FN) == 0, NA, TP / (TP + FN)) #measure missed detection rate
...
metrics_df <- tibble(
  ...
  left_join(cs_raw, by = "Class") %>% # to add number of annotations for each class
  arrange(desc(F1))
...
metrics_df %>%
  ...
  scale_color_gradientn(
    colours = c("#A32D2D", "#F09595", "#BA7517", "#1D9E75", "#0F6E56"),
    values  = scales::rescale(c(0, 0.25, 0.50, 0.75, 1)),
    limits  = c(0, 1),
    breaks  = c(0.25, 0.50, 0.75),
    labels  = c("25% — Poor", "50% — Moderate", "75% — Good"),
    name    = "F1 score"
  )+
